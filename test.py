...
class Human:
    ...
    def petting(self):
        if self.pet:
            self.satiety += 10
            self.pet.mood += 5
            print(f"{self.name} stroked {self.pet.pet}")
        else:
    ...
    def live(self, day):
        ...
        if self.home == "None":
            ...
            self.get_home()
        if self.car == "None":
            self.get_car()  # Initialize car here if it is None
            print(f"I bought a car {self.car.brand}")
        elif self.job == "None":
            self.get_job()
            print(f"Going to get job {self.job.job}, with salary {self.job.salary}")
...
class Pet:
    def __init__(self, pet_list):
        ...
        self.satiety = pet_list[self.pet]["satiety"]
        self.mood = pet_list[self.pet]["mood"]
